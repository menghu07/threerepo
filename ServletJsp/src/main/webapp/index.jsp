...
<p>JSP Scriptlet:<%=pageContextApp%> servletPath=<%=request.getServletPath()%> <br>
    contextPath=<%=request.getContextPath() %><br>URI=<%=request.getRequestURI()%><br>URL=<%=request.getRequestURL()%>
</p>
<p>EL id=${id} servletPath=${pageContext.request.servletPath} requestScope servletPath =
    servletPath=${requestScope.clear()} clear之后requestScope=${requestScope.entrySet()}</p>
<p>JSP Scriptlet variable 任意范围的属性pageContextApp=${pageContextApp} pageScope requestScope sessionScope applicationScope
    作用范围逐渐增大，其方法的是属性</p>
...
<script>
    (function () {
        $.ajax({
            url: "/ServletJsp/pages/chapter04/ElExpression.ch04do", success: function (data) {
                console.log(data);
            }
        });
    })();
